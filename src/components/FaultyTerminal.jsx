...
export default function FaultyTerminal({
  ...
  const pageLoadProgressRef = useRef(pageLoadAnimation ? 0 : 1);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error('OGL Renderer initialization failed:', err);
      return;
    }
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uUseMouse: { value: mouseReact ? 1.0 : 0.0 },
        uPageLoadProgress: { value: pageLoadProgressRef.current },
        uUsePageLoadAnimation: { value: pageLoadAnimation ? 1.0 : 0.0 },
    ...
    function update(t) {
      ...
      if (!pause) {
        frozenTimeRef.current = t * 0.001 * timeScale;
      }
      ...
      if (pageLoadAnimation && pageLoadProgressRef.current < 1.0) {
        pageLoadProgressRef.current = Math.min(1.0, pageLoadProgressRef.current + 0.01);
        program.uniforms.uPageLoadProgress.value = pageLoadProgressRef.current;
      }
